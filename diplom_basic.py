...
class CopyPhotosFromVKtoYADisk:
    ...
    def get_likes_date(self):
        if count == 0 or count > len(data_list):
            times = len(data_list)
        else:
            times = count
        i = 1
    ...
    def get_links(self):
        ...
        for item in data_list:
            for dictionary in item['sizes']:
                height_list.append(dictionary['type'])
                if dictionary['type'] == 'w':
                    break
            if dictionary['type'] in reversed(height_list[-1]):
    ...
    def get_filenames(self):
        for link in self.link_list:
            for date, like in self.date_link_dict.items():
                http_filename = str(link).split('/')[-1]
                filename = (http_filename.split('.')[0]).replace(
                    http_filename.split('.')[0], str(like) + '.jpg'
                )
    ...
    def put_data_to_ya(self):
        ...
        for filename in tqdm(self.filename_list):
            while i < len(self.link_list):
                response_from_link = requests.get(self.link_list[i])
                i += 1
                break
